...
pub fn funcs_parser() -> impl Parser<Token, HashMap<String, Func>, Error = Simple<Token>> + Clone {
	...
	let func = just(Token::Fn)
		...
		.map_with_span(|((name, args), body), span| (name.clone(), Func { args, body, name, span }))
		.labelled("function");
		...
		.try_map(|fs, _| {
			...
			for ((name, name_span), f) in fs {
				if funcs.insert(name.clone(), f).is_some() {
					return Err(Simple::custom(name_span, format!("Function '{}' already exists", name)));
				}
			}
			Ok(funcs)
		})
...
pub fn type_inference(expr: &Spanned<Expr>, symbol_type_table: &mut HashMap<Span, Value>) {
	match &expr.0 {
		Expr::Error => {}
		Expr::Value(_) => {}
		Expr::List(exprs) => exprs.iter().for_each(|expr| type_inference(expr, symbol_type_table)),
		Expr::Local(_) => {}
...
pub fn parse(
	...
	let parse_errors = errs
		...
		.chain(tokenize_errors.into_iter().map(|e| e.map(|tok| tok.to_string())))
		.collect::<Vec<_>>();
