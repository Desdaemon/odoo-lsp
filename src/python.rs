...
impl Backend {
	...
	fn gather_commandlist<'text>(
		...
		debug!("Initial dest: {:?}", dest.map(|n| (n.kind(), n.byte_range())));
		...
		if dest.is_none() && offset > cmdlist.start_byte() {
			debug!("No node at offset {}, trying offset - 1", offset);
			...
			if let Some(node) = cmdlist.descendant_for_byte_range(offset - 1, offset - 1) {
					...
					|| node.kind() == "string_end"
				{
					...
					if let Some(next_sibling) = string_node.next_sibling() {
						debug!("String has next sibling: {}", next_sibling.kind());
						if next_sibling.kind() != ":" {
							// This is an incomplete field name, provide completions
							dest = Some(string_node);
						}
					} else {
						debug!("String has no next sibling, treating as incomplete");
		...
		if let Some(parent) = dest.parent() {
			...
			if parent.kind() == "dictionary" {
				...
				if let Some(grandparent) = parent.parent() {
					debug!("ERROR parent (grandparent) kind: {}", grandparent.kind());
		...
		if is_broken_syntax {
			debug!("Detected broken syntax: string in dictionary without colon");
		...
		while count < 30 {
			...
			let Some(candidate) = cursor.child_with_descendant(dest) else {
				debug!("child_containing_descendant returned None at count={}", count);
				return None;
			};
			let obj;
			debug!("candidate kind: {}", candidate.kind());
			...
			if obj.kind() == "dictionary" {
				...
				if pair.kind() != "pair" {
					...
					debug!("Returning None: pair kind {} is not 'pair'", pair.kind());
					return None;
				}
		...
		debug!("Access path: {}", access);
		debug!("Initial model before resolve: {}", model_str);
		let access = &mut access.as_str();
		let mut model = _I(model_str);
		if self.index.models.resolve_mapped(&mut model, access, None).is_err() {
			debug!("resolve_mapped failed for model={} access={}", _R(model), access);
			return None;
		}
