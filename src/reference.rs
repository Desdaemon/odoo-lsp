...
use ReferenceSymbol::*;
pub fn get_reference(ast: &HashMap<String, Func>, ident_offset: usize, include_self: bool) -> Vec<Spanned<String>> {
	...
	match &expr.0 {
		...
		Expr::Local((name, span)) => {
			if let Founded((symbol_name, symbol_span)) = reference_symbol {
				if &symbol_name == name {
					let index = definition_ass_list.iter().position(|decl| decl.0 == symbol_name);
		...
		Expr::Binary(lhs, _op, rhs) => {
			...
			get_reference_of_expr(rhs, definition_ass_list, reference_symbol, reference_list, include_self);
		}
