	...
	match &expr.0 {
		...
		Expr::Local(local) => {
			if ident_offset >= local.1.start && ident_offset < local.1.end {
				let index = definition_ass_list.iter().position(|decl| decl.0 == local.0);
				(false, index.map(|i| definition_ass_list.get(i).unwrap().clone()))
			} else {
				(true, None)
			}
		}
		Expr::Let(name, lhs, rest, name_span) => {
			...
			match get_definition_of_expr(lhs, definition_ass_list.clone(), ident_offset) {
				(true, None) => get_definition_of_expr(rest, new_decl + definition_ass_list, ident_offset),
				(true, Some(value)) => (false, Some(value)),
				(false, None) => (false, None),
				(false, Some(value)) => (false, Some(value)),
			}
		}
		Expr::Then(first, second) => match get_definition_of_expr(first, definition_ass_list.clone(), ident_offset) {
		...
		Expr::Binary(lhs, _, rhs) => match get_definition_of_expr(lhs, definition_ass_list.clone(), ident_offset) {
