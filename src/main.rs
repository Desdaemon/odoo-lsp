        ...
        match self.xml_completions(params, document.value()).await {
            ...
            Err(report) => {
                self.client
                    .show_message(
                        MessageType::ERROR,
                        format!("error during completion:\n{report}"),
                    )
                    .await;
                Ok(None)
            }
...
fn read_to_end(
    lex: &mut Tokenizer,
    local_name: &str,
) -> miette::Result<Option<std::ops::Range<CharOffset>>> {
    let mut stack = 0;
    loop {
        match lex.next() {
            Some(Ok(Token::ElementStart { local, .. })) if local.as_str() == local_name => {
                stack += 1
            }
...
impl Backend {
    async fn on_change(&self, params: TextDocumentItem) {
        ...
        let rope = params
            .rope
            .unwrap_or_else(|| ropey::Rope::from_str(&params.text));
        if matches!(split_uri, Some((_, "xml"))) || matches!(params.language, Some(Language::Xml)) {
            ...
            self.record_ranges.insert(
                params.uri.path().to_string(),
                record_ranges.into_boxed_slice(),
            );
        }
        self.client
    ...
    async fn xml_completions(
        ...
        let position = params.text_document_position.position;
        let Some(ranges) = self.record_ranges.get(params.text_document_position.text_document.uri.path()) else {panic!("Could not get record ranges")};
        let Some(cursor) = position_to_offset(position, rope) else {panic!("cursor")};
        let Ok(mut cursor_by_char) = rope.try_byte_to_char(cursor.0) else {panic!("cursor_by_char")};
        let Ok(record) = ranges.value().binary_search_by(|range| {
            ...
            eprintln!("Could not find record for cursor={cursor:?} ranges={:?}", ranges.value());
            return Ok(None)
        };
        let record_range = &ranges.value()[record];
        let Ok(relative_offset) = rope.try_byte_to_char(record_range.start.0) else {panic!("relative_offset")};
                ...
                Ok(Token::Attribute { local, value, .. })
                    if local.as_str() == "ref"
                        && (value.range().contains(&cursor_by_char)
                            || value.range().end == cursor_by_char) =>
                {
                    let needle = &value.as_str()[..cursor_by_char - value.range().start];
                    let replace_range = value.range();
                    let Some(replace_start) = char_offset_to_position(replace_range.start + relative_offset, rope) else {panic!("replace_start")};
                    let Some(replace_end) = char_offset_to_position(replace_range.end + relative_offset, rope) else {panic!("replace_end")};
...
impl Record {
        ...
        S: Into<FastStr>,
    {
        ...
        loop {
            match reader.next() {
                ...
                Some(Ok(Token::ElementStart { local, .. })) if local.as_str() == "field" => {
                    ...
                    let Some(maybe_inherit_id) = maybe_inherit_id else {continue};
                    if let Some((module, xml_id)) = maybe_inherit_id.split_once('.') {
                        inherit_id =
                            Some((Some(module.to_string().into()), xml_id.to_string().into()));
                    } else {
        ...
        let end = char_offset_to_position(end, rope)
            .ok_or_else(|| diagnostic!("Failed to parse end location"))?;
